...
export default function UploadMedComponent() {
  ...
  const handleFormSubmit = async () => {
    if (!uploadForm.issue || !uploadForm.doctorName || !uploadForm.file) {
      alert('Please fill in all fields and select a file');
      return;
    }

    try {
      const response = await axios.post('/api/medicine/upload', uploadForm);
